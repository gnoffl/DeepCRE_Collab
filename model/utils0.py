...
class ConvNetwork:
    ...
    def build_network(self, x_train, x_val, y_train, y_val):
        ...
        print(model.summary())
        if self.effect_size:
            model_save_name = f'saved_models/size_effect/{self.organism}_{self.val_chrom}_{self.outer}_{self.inner}.keras'
        else:
            model_save_name = f'saved_models/{self.organism}_model_{self.val_chrom}_{self.case}{self.tissue}.keras'
        ...
        if model_save_name.endswith(f'si-nucleotide_shuffle{self.tissue}.keras'):
